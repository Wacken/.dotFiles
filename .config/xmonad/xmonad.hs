...
myLayout
  :: ModifiedLayout
       ...
       ( ModifiedLayout
           AvoidStruts
           ( Choose
               (ModifiedLayout Rename ResizableTall)
               (Choose (Mirror (ModifiedLayout Rename ResizableTall)) Full)
           )
       )
       a
myLayout = smartBorders . avoidStruts $ myDefaultLayout
  where myDefaultLayout = (tall ||| Mirror tall ||| Full)
